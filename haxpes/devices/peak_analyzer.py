...
class PeakAnalyzer(Device):
    ...
    measurement_state = Cpt(
        Signal, name="measurement_state", kind="config", value="Ready"
    )
    ...
    def _getparameters(self):
        ...
        self.yaxis.put(
            np.linspace(
                start=self._acqclient.current_y_axis.minimum,
                stop=self._acqclient.current_y_axis.maximum,
                num=self._acqclient.current_y_axis.count,
            )
        )
    ...
    def stage(self):
        ...
        self.set_live_lens_mode(self.lens_mode.get())
    ...
    def _acquire(self, status):
        ...
        self.xaxis.put(
            np.linspace(
                start=specdat.x_axis.minimum,
                stop=specdat.x_axis.maximum,
                num=specdat.x_axis.count,
            )
        )
        self.yaxis.put(
            np.linspace(
                start=specdat.y_axis.minimum,
                stop=specdat.y_axis.maximum,
                num=specdat.y_axis.count,
            )
        )
        ...
        try:
            ...
            self.y_CoM.put(
                50
            )  # CoM greater than yaxis size; this will occur when sample is too far away to get counts
        self.opt_val.put(
            specdat.data.sum() - self.opt_par.get() * np.abs(self.y_CoM.get())
        )
    ...
    def setup_from_dictionary(self, region_dictionary, analyzer_settings, **kwargs):
        ...
        if "scan_type" in kwargs.keys():
            ...
            scan_type = "XPS" #default to XPS mode

        if scan_type == "XPS":
            self.setsweptmode()
            self.energy_center.put(region_dictionary["energy_center"])
            self.energy_step.put(region_dictionary["energy_step"])
            self.energy_width.put(region_dictionary["energy_width"])
        ...
        self.region_name.put(region_dictionary["region_name"])
        if "description" in region_dictionary.keys():
            self.description.put(region_dictionary["description"])

        self.pass_energy.put(analyzer_settings["pass_energy"])
        if "dwell_time" in analyzer_settings.keys():
            dwelltime = analyzer_settings["dwell_time"]
        else:
            dwelltime = default_dwell_time
        self.dwell_time.put(dwelltime)
        if "lens_mode" in analyzer_settings.keys():
            lensmode = analyzer_settings["lens_mode"]
        else:
            lensmode = default_lens_mode
        self.lens_mode.put(lensmode)
        if "acquisition_mode" in analyzer_settings.keys():
            acqmode = analyzer_settings["acquisition_mode"]
        else:
    ...
    def set_live_energy_center(self, kinetic_energy):
        if self._livemode:
            self._anclient.set_analyser_energy(
                kinetic_energy, PeakEnergyMode("Kinetic")
            )
